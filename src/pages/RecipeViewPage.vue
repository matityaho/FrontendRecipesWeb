    ...
    <div v-if="recipe">
      ...
      <b-card class="recipe-body">
        <div class="wrapper">
          <div class="wrapped">
            <div class="mb-3">
              <div v-if="recipe.favorite != null && recipe.watched != null">
                <b-icon-eye-fill
                  v-b-tooltip.hover
                  title="Already show this recipe"
                  v-if="recipe.watched"
                />
                <b-icon-eye
                  v-b-tooltip.hover
                  title="Never seen this recipe"
                  v-else
                />
                &nbsp;
                <b-icon-heart-fill
              ...
              {{ recipe.likesAmount }}
              <br />
              <div>Dish Amount: {{ recipe.dishAmount }} dishes</div>
              <img
                v-if="recipe.vegan"
                v-b-tooltip.hover
                title="Vegan"
...
export default {
  ...
  methods: {
    async updateFavorite() {
      ...
      let ans = await this.axios.get(
        "http://localhost:3100/users/addRecipeToFavorites/" + this.recipe.id
      );
    },
  },
    ...
    try {
      ...
      try {
        response = await this.axios.get(
          "http://localhost:3100/recipes/Information/" +
            this.$route.params.recipeId
        );
        if (this.$cookies.get("session") != null) {
          let ans = await this.axios.get(
            "http://localhost:3100/users/addRecipeToWatched/" +
              this.$route.params.recipeId
          );
        }
